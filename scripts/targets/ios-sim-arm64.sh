...
#
# The iOS simulator on an Apple Silicon Mac.
#
# The simulator runs the host architecture, so this is the pair for ios-arm64
# on Apple Silicon, exactly as ios-sim-x86_64 is on Intel.
TARGET_CROSS=1
...
TARGET_CFLAGS_EXTRA="-arch arm64 -isysroot $TARGET_SYSROOT -mios-simulator-version-min=$TARGET_MIN_VERSION"
...
# Which libvpx target to configure. as the device, against the simulator SDK
